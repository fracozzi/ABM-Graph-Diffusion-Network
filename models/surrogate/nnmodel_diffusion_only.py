    ...
    def train(self, ramifications, n_ramifications = 500, n_epochs=10):
        ...
        for epoch in tqdm(range(n_epochs), desc="Training"):
            for x_t, edge_index, y_t in dataloader:
                ...
                # Domain condition takes full state (static + dynamic)
                domain_condition = x_t[0].flatten().unsqueeze(0).expand(n_agents, -1)
                ...
                loss = self.p_losses(
                    x_start = y_t[0],
                    state=prev_state_condition,
                    label=domain_condition,
                    t=t_diffusion,
...
class SimulatedABM_diffusion_only(ABM):
    """ Class that takes a trained model and puts into the same interface used by ABM.
    """
    def __init__(self, initial_state: np.ndarray, model: NNModel_diffusion_only):
        self.model = model
